...
bool rayIntersectDepth(float rayZNear, float rayZFar, vec2 hitPixel)
{
    ...
    // Cross z
    // PENDING Too trick
    return rayZFar <= cameraZ && rayZNear >= min(cameraBackZ, cameraZ) - zThicknessThreshold;
}
...
void main()
{
    ...
    vec3 hitNormal = texture2D(normalTex, hitPixel).rgb * 2.0 - 1.0;
...
void main()
{
    vec4 sourceTexel = texture2D(colorTex, v_Texcoord);

#ifdef SOURCE_RGBM_DECODE
    sourceTexel = decodeHDR(sourceTexel);
#else
    sourceTexel.a = 1.0;
#endif

    gl_FragColor = encodeHDR(
        decodeHDR(texture2D(ssrTex, v_Texcoord)) * texture2D(ssaoTex, v_Texcoord).r + sourceTexel
    );
}
